...
def main():
    args = sys.argv
    cancer_type = args[1]
    if(cancer_type[-2] == '_'): true_cancer_type = cancer_type[:-2]
    else: true_cancer_type = cancer_type[:-3]
    mutation,upstream,downstream = load_data(true_cancer_type)
    up_ngram, up_posterior_sampling = load_result(cancer_type, true_cancer_type, 'Up')
    down_ngram, down_posterior_sampling = load_result(cancer_type, true_cancer_type, 'Down')
...
def load_result(cancer_type, true_cancer_type,  direction):
    file_name = 'result/' + cancer_type + '/corpus_' + true_cancer_type + '_' + direction + '.txt'
...
def draw_fig_ngram(direction, ngram, cancer_type):
    ...
    fig.tight_layout()
    fig.savefig('result/' + cancer_type + '/ngram_count_' + direction + '.png', dpi=300)
    plt.close(1)
...
def draw_fig_ngram_by_mutation(direction, mutation, ngram, cancer_type, context):
    ...
    for i in range(len(mutation)):
        new_ngram[mutation[i]].append(ngram[i])
    ...
    base_list = ['A', 'C', 'G', 'T']
    for i in range(len(mutation)):
        if(max_ngram[mutation[i]] != -1): height[mutation[i], context[i][max_ngram[mutation[i]]]] += 1
    fig = plt.figure()
    ax1 = fig.add_subplot(2,3,1); ax2 = fig.add_subplot(2,3,2); ax3 = fig.add_subplot(2,3,3)
    ax4 = fig.add_subplot(2,3,4); ax5 = fig.add_subplot(2,3,5); ax6 = fig.add_subplot(2,3,6)
    axs = plt.gcf().get_axes()
