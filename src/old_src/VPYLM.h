...
public:
    ...
    void calc_argument(Node* start_node, vector<double> &dis1, vector<double> &dis2, vector<double> &theta1, vector<double> &theta2);
...
VPYLM::VPYLM(string x, string y, vector<int> &z, vector<vector<int> > &w, int v){
    cancer_type = x;
    direction = y;
    mutation = z;
    context = w;
    ...
    for (int i=0; i < MAX_CONTEXT_NUM+1; i++){
        ...
        theta.push_back(sampler::uniform(-dis[i] + 1.000000, -dis[i] + 2.000000));
    }
...
void VPYLM::run_learning(){
    ...
    for (int iter=0; iter < 200; iter++){
        ...
        calc_log_likelihood();
        cout << "temp log_likelihood : " << to_string(temp_log_likelihood) << endl;
        cout << "Improved : " << to_string(temp_log_likelihood - old_log_likelihood) << endl;
...
void VPYLM::make_suffix_tree(){
    for (int i=0; i < num_mutation; i++){
        ...
        for (int j=0; j < MAX_CONTEXT_NUM; j++){
            ...
            if(parent -> exist_next_node(base)) parent = parent -> find_next_node(base);
            else{
                Node *node = new Node();
...
void VPYLM::initialize(){
    for (int i=0; i < num_mutation; i++){
        ...
        while(j < MAX_CONTEXT_NUM+1){
            int stop_flag = sampler::bernoulli(parent -> initial_stop_probability);
            if(stop_flag == 1){
...
void VPYLM::add_customer(int index, Node* node){
    ...
    if(node != root){
        ...
        for (int i=0; i < node -> num_all_table; i++){
            double candidate = node -> count[temp_mutation][i] - dis[dep] * node -> table[temp_mutation];
...
void VPYLM::calc_p_m(Node* start_node){
    ...
    if(start_node != root){
        ...
        for (int i=0; i < start_node -> num_vocabulary; i++){
            start_node -> p_m[i] = count_w[i] - dis[dep] * start_node -> table[i] 
                                + (theta[dep] + dis[dep] * start_node -> num_all_table) * parent_p_m[i];
        }
    ...
    for (int i=0; i < start_node -> children.size(); i++){
        ...
        if((node -> a != 0) || (node -> b != 0)) calc_p_m(node);
    }
}
...
void VPYLM::reset_ab(int index, Node* node){
    if (node == real_customer_arrangement[index]) node -> a--;
    else node -> b--;
...
void VPYLM::arrange_real_customer(int index){
    ...
    for(int i=0; i < MAX_CONTEXT_NUM+1; i++){
        ...
        for (int j=0; j < i; j++){
            prod_pass *= (node -> b + node -> beta)/(node -> a + node -> b + node -> alpha + node -> beta);
            node = node -> find_next_node(context[index][j]);
        }
        double prob = (node -> a + node -> alpha)/(node -> a + node -> b + node -> alpha + node -> beta) * prod_pass;
...
void VPYLM::update_hyperparameter(){
    ...
    vector<double> dis_argument1, dis_argument2, theta_argument1, theta_argument2;
    dis_argument1.resize(MAX_CONTEXT_NUM+1, Kappa); dis_argument2.resize(MAX_CONTEXT_NUM+1, Lambda);
    theta_argument1.resize(MAX_CONTEXT_NUM+1, Mu); theta_argument2.resize(MAX_CONTEXT_NUM+1, Nu);
    calc_argument(root, dis_argument1, dis_argument2, theta_argument1, theta_argument2);
    for (int i=0; i < MAX_CONTEXT_NUM+1; i++){
...
void VPYLM::update_auxiliary_variables(Node* start_node){
    ...
    for (int i=0; i < start_node -> children.size(); i++){
        ...
        if((next_node -> a != 0) || (next_node -> b != 0)) update_auxiliary_variables(next_node);
    }
}

void VPYLM::calc_argument(Node* start_node, vector<double> &dis1, vector<double> &dis2, vector<double> &theta1, vector<double> &theta2){
    ...
    for (int i=0; i < start_node -> children.size(); i++){
        Node *next_node = start_node -> children[i];
        if((next_node -> a != 0) || (next_node -> b != 0)){
...
void VPYLM::calc_log_likelihood(){
    ...
    for (int i=0; i < num_mutation; i++){
        ...
        temp_log_likelihood += log(real_customer_arrangement[i] -> p_m[temp_mutation]);
    }
...
void VPYLM::write_data(){
    ...
    cout << "------------------" << endl;
    ofstream ofs;
    string output_file_name = "result/" + cancer_type + "_" +  to_string(exp_index)+ "/corpus_" + cancer_type + "_" + direction + ".txt";
    ...
    ofs.close();
    ofs.open("result/" + cancer_type + "_" + to_string(exp_index) + "/corpus_" + cancer_type + "_" + direction + "_log_likelihood.txt", ios::out);
...
string VPYLM::posterior_sampling(int mutation){
    ...
    for (int i=0; i < MAX_CONTEXT_NUM+1; i++){
        double stop_probability = (temp_node -> a + temp_node -> alpha)/(temp_node -> a + temp_node -> alpha + temp_node -> b + temp_node -> beta);
        if(sampler::bernoulli(stop_probability) || (i == MAX_CONTEXT_NUM)) break;
        ...
        if((next_node -> a == 0) && (next_node -> b == 0)) break;
...
void VPYLM::print_root_count(int iter, int index, string status){
    cout << "iter : " << to_string(iter) << ", index : " << to_string(index) << ", mutation : " << to_string(mutation[index]) << endl;
